    ...
    public static TreeSet<Order> toTreeSet(List<Order> orders) {
        return orders.stream()
                .collect(Collectors.toCollection(
                        () -> new TreeSet<>(Comparator.comparing(Order::isOpen).thenComparing(Order::getAmount)
                                .reversed().thenComparing(Order::getOrderTime).thenComparing(Order::getId))));
    }

    public static void main(String[] args) {
        Instant now = Instant.now();
        System.out.println(toTreeSet(Arrays.asList(new Order(1, now, false, new BigDecimal("1")),
