...
public class Order   {
    ...
    public static TreeSet<Order> toTreeSet(List<Order> orders) {
        final TreeSet<Order> collect = orders.stream()
                .collect(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(Order::isOpen)
                        .thenComparing(Order::getAmount).reversed().thenComparing(Order::getOrderTime).thenComparing(Order::getId))));
        return collect;
    }
