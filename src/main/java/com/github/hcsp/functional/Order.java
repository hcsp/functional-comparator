...
public class Order {
    ...
    public static TreeSet<Order> toTreeSet(List<Order> orders) {
        Comparator<Order> cmp = Comparator.comparing((Order order)->order.open? 1 : -1)
                            .thenComparing((Order order)->order.amount).reversed()
                            .thenComparing((Order order)-> order.orderTime)
